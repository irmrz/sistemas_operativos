...
scheduler(void)
{
  ...
  int xticks;
  int low_level_quantity;
  
  bool avoid_starvation = false;
  bool pick_proc = false; 
  c->proc = 0;
  ...
  for(;;){
    ...
    acquire(&tickslock);
    xticks = ticks;
    if (xticks % STARVATION_TICKS == 0){
      low_level_quantity = levels_mlfq[LOW_PRIO].used_size;
      avoid_starvation = true;
    }
    release(&tickslock);
    
    pick_proc = false;
    for (int j = 0; j < NPRIO; j++){
      if (levels_mlfq[j].used_size > 0) {
          
          //Pick a process from queue priority j
          p = dequeue(&levels_mlfq[j]);
          acquire(&p->lock);
          release(&sch);       

          //Avoid starvation 
          if (avoid_starvation && (j == LOW_PRIO)){
            p->priority = HIGH_PRIO;
            low_level_quantity--;
            avoid_starvation = low_level_quantity > 0;
          }
          ...
          // Increment times that scheduler choose x proccss
          p->scheduler_chossen++;
