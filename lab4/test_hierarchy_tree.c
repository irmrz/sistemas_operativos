...
static h_tree
add_elem_list(h_tree tree, int size, char** elem_array)
{
  void* new_elem = NULL;
...
static h_tree
add_test_elems(h_tree tree)
{
  return add_elem_list(tree, 7, test_elems);
}

static void
compare_preorder_list(h_tree tree, char** expected_elems)
{
