...
static int
read_dos_2_0_bpb(fat_volume vol, const struct fat_boot_sector_disk* boot_sec)
{
...
static int
read_dos_3_31_bpb(fat_volume vol, const struct fat_boot_sector_disk* boot_sec)
{
...
static int
read_nonfat32_ebpb(fat_volume vol, const struct nonfat32_ebpb* ebpb)
{
...
static int
read_fat32_ebpb(fat_volume vol, const struct fat32_ebpb* ebpb)
{
 ...
 */
static int
read_boot_sector(fat_volume vol, int fd)
{
...
static int
map_fat(fat_volume vol, int fd, int mount_flags)
{
  long page_size;
  void* ptr;
  ...
  ptr = mmap(
    NULL, fat_aligned_size_bytes, prot, MAP_PRIVATE, fd, fat_aligned_offset);
  if (ptr == MAP_FAILED)
    return -1;
...
fat_volume
fat_volume_mount(const char* volume, int mount_flags)
{
  fat_volume vol = NULL;
  int fd;
  int ret;
...
int
fat_volume_unmount(fat_volume vol)
{
  int ret;
