...
void
fat_tree_destroy(fat_tree tree)
{
  if (tree != NULL) {
...
int
fat_tree_size(const fat_tree tree)
{
  if (tree == NULL) {
    return -1;
  }
...
fat_tree
fat_tree_insert(fat_tree tree, fat_tree_node parent, const fat_file new_file)
{
  if (tree == NULL || new_file == NULL) {
    errno = EINVAL;
    return NULL;
  }
  tree->file_tree = h_tree_insert(
    tree->file_tree, (void*)new_file, (h_tree)parent, tree->file_cmp);
  return tree;
}

fat_file
fat_tree_search(const fat_tree tree, const char* key)
{
...
fat_tree_node
fat_tree_node_search(const fat_tree tree, const char* key)
{
  return h_tree_search(tree->file_tree, (void*)key, tree->file_cmp_key);
}

fat_file
fat_tree_get_file(const fat_tree_node node)
{
  return h_tree_get_data(node);
}

fat_file
fat_tree_get_parent(const fat_tree_node node)
{
  return (fat_file)h_tree_get_data(h_tree_get_h_parent(node));
}

void
fat_tree_inc_num_times_opened(fat_tree_node node)
{
...
void
fat_tree_dec_num_times_opened(fat_tree_node node)
{
  h_tree_iterate_h_ancestors(node,
...
fat_tree
fat_tree_delete(fat_tree tree, const char* key)
{
  if (tree == NULL) {
    errno = EINVAL;
    return NULL;
  ...
  tree->file_tree =
    h_tree_delete(tree->file_tree, key, tree->file_cmp_key, tree->file_destroy);
  return tree;
}

void
fat_tree_iterate_preorder(fat_tree tree, data_modify_fn mod_fn)
{
  if (tree != NULL) {
...
void
fat_tree_print_preorder(fat_tree tree)
{
  if (tree != NULL) {
...
void**
fat_tree_flatten_preorder(const fat_tree tree)
{
...
fat_file*
fat_tree_flatten_h_children(const fat_tree_node dir_node)
{
