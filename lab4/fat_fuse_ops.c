...
u32 fs_log_start_cluster = 0;
u32 orphan_cluster = 0;
fat_dir_entry bb_entry = NULL;
int crt = 0;
...
static void
fat_fuse_log_activity(char* operation_type, fat_file file)
{
  ...
  fat_tree_node fs_node = fat_tree_node_search(vol->file_tree, BB_LOG_FILE);
  
  if (fs_node == NULL){
    u32 bytes_per_cluster = 0;
    off_t offset = 0;
    fat_dir_entry buf2 = NULL;

    bytes_per_cluster = fat_table_bytes_per_cluster(vol->table);
    buf2 = alloca(bytes_per_cluster);

    offset = fat_table_cluster_offset(vol->table, orphan_cluster);
    full_pread(vol->table->fd, (void*)buf2, sizeof(struct fat_dir_entry_s), offset);

    fs_log_start_cluster = file_start_cluster(buf2);
  
    new_file = fat_file_init(vol->table, false, BB_LOG_FILE);
    if (crt)
      new_file->start_cluster = fs_log_start_cluster;
    else
      new_file->start_cluster = fs_log_start_cluster + buf2->file_size;
    fat_tree_insert(vol->file_tree,dir_node,new_file);
  }
  else{
    fs_node = fat_tree_node_search(vol->file_tree, BB_LOG_FILE); 
    new_file = fat_tree_get_file(fs_node);
  }
  
  if (fat_file_pwrite(new_file, buf, message_size, new_file->dentry->file_size, dir) < 0)
...
int
fat_fuse_fgetattr(const char* path,
                  struct stat* stbuf,
 ...
 */
static void
fat_fuse_read_children(fat_tree_node dir_node)
{
  fat_volume vol = get_fat_volume();
  fat_file dir = fat_tree_get_file(dir_node);
  ...
  /* Save cluster number as global var */
  orphan_cluster = orphan_cluster_nummer;
...
int
fat_fuse_readdir(const char* path,
                 void* buf,
...
int
fat_fuse_write(const char* path,
               const char* buf,
               size_t size,
               off_t offset,
               struct fuse_file_info* fi)
...
int
fat_fuse_truncate(const char* path, off_t offset)
{
  errno = 0;
  fat_volume vol = get_fat_volume();
...
int
fat_fuse_unlink(const char* path)
{
  errno = 0;
  fat_volume vol = get_fat_volume();
...
int
fat_fuse_rmdir(const char* path)
{
  errno = 0;
  fat_volume vol = get_fat_volume();
