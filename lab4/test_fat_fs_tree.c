...
static fat_tree
insert_many_files(fat_tree tree, int n, char** filenames)
{
...
static void
compare_tree_preorder(fat_tree tree, char** filenames)
{
...
START_TEST(test_increase_time_opened)
{
  ...
  fail_unless(fat_file_cmp(fat_tree_get_file(node11), (fat_file) "X1.1") == 0);
  fail_unless(fat_file_cmp(fat_tree_get_file(node1), (fat_file) "X1") == 0);
  // Nodes that should have not been visited
  fail_unless(fat_file_cmp(fat_tree_get_file(node12), (fat_file) "X1.2") != 0);
