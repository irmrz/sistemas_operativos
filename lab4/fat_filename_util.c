...
static bool
char_legal_in_filename(char c)
{
  return (_fat_char_tab[(unsigned char)c] & FAT_CHAR_LEGAL_IN_FILENAME) != 0;
}

bool
file_basename_valid(const u8 base_name[8])
{
  unsigned i;
...
bool
file_extension_valid(const u8 extension[3])
{
  unsigned i;
...
unsigned
filename_len(const char* name, unsigned max_len)
{
...
void
build_filename(const u8* src_name_p,
               const u8* src_extension_p,
               char* dst_name_p)
{
  unsigned name_len;
...
void
filename_from_path(char* src_name_p, u8* base, u8* extension)
{
...
char*
filepath_from_name(char* parent_filepath, char* file_name)
{
