...
static void
write_dir_entry(fat_file parent, fat_file file);
...
static int
fill_dentry_time_now(fat_dir_entry dir_entry,
                     bool fill_create,
                     bool fill_modified)
{
  int ret = 0;
 ...
 * Copied from libfat. */
static int
set_first_cluster(fat_dir_entry dir_entry, u32 cluster)
{
  if (dir_entry == NULL)
    return -1;
...
static fat_file
fat_file_init_empty(bool is_dir, char* filepath)
{
...
fat_file_init_orphan_dir(char* name, fat_table table, u32 start_cluster)
{
  ...
  build_filename(orphan_entry->base_name,
                 orphan_entry->extension,
...
fat_file_init_direntry(bool is_dir, char* filepath, u32 start_cluster)
{
  ...
  filename_from_path(
    basename(strdup(filepath)), new_entry->base_name, new_entry->extension);
  if (is_dir) {
 ...
 */
static fat_dir_entry
init_direntry_from_buff(fat_dir_entry buff)
{
  fat_dir_entry new_entry = calloc(1, sizeof(struct fat_dir_entry_s));
  if (new_entry == NULL) {
    errno = ENOSPC;
...
static fat_file
init_file_from_dentry(fat_dir_entry dentry, fat_file parent)
{
  fat_file new_file = NULL;
  ...
  build_filename(
    dentry->base_name, dentry->extension, (char*)&(new_file->name));
...
fat_file_init(fat_table table, bool is_dir, char* filepath)
{
  ...
  build_filename(new_file->dentry->base_name,
                 new_file->dentry->extension,
                 (char*)&(new_file->name));
...
write_dir_entry(fat_file parent, fat_file file)
{
  ...
  DEBUG("Writting dentry on directory %s, entry %u",
        parent->name,
        file->pos_in_parent);
...
fat_file_dentry_add_child(fat_file parent, fat_file child)
{
  ...
  DEBUG("Adding child \"%s\" to \"%s\" in position %u",
        child->name,
        parent->filepath,
        parent->dir.nentries);
  parent->dir.nentries++;
...
static bool
is_end_of_directory(const fat_dir_entry disk_dentry)
{
...
static bool
ignore_dentry(const fat_dir_entry disk_dentry)
{
 ...
 */
static void
read_cluster_dir_entries(u8* buffer,
...
fat_file_truncate(fat_file file, off_t offset, fat_file parent)
{
  ...
  fat_table_set_next_cluster(
    file->table, last_cluster, FAT_CLUSTER_END_OF_CHAIN_MAX);
 ...
 */
static u32
next_or_new_cluster(fat_file file, u32 last_cluster)
{
                ...
                fat_file parent)
{
  ...
  cluster =
    fat_table_seek_cluster(file->table,
                           file->start_cluster,
